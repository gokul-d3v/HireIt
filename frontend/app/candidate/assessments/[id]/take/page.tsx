...
interface Question {
    id: string; // Assuming we use index for now if ID is missing from API
    text: string;
...
export default function TakeAssessmentPage() {
    ...
    }, [timeLeft]); // Dependency on timeLeft to trigger auto-submit logic correctly
    ...
    const handleAnswerChange = (value: string) => {
        // Use question ID if available, otherwise use ID from question object (assuming API provides it) or fallback
        // For this implementation, we'll assume questions have IDs or we use something unique. 
        // Ideally backend provides IDs. If not, we might need to rely on index (risky if reordered)

        // Let's assume assessment.questions[currentQuestionIndex].id exists from backend model
    ...
    const submitAssessment = async (autoSubmit = false) => {
        ...
        try {
            // Format answers for backend: [{question_id: "...", value: "..."}]
            ...
            setSubmitting(false); // Only reset if failed so user can try again
        }
    };

    if (loading || !assessment) return <div className="p-8 flex justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600"></div></div>;
                        ...
                        <div className="space-y-4">
                            ...
                            {(currentQuestion.type === "SUBJECTIVE" || currentQuestion.type === "CODING") && (
                                <textarea
                                    ...
                                    className="w-full h-64 p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm leading-relaxed"
